...
use massa_models::{
    operation::{OperationId, OperationIds, SignedOperation},
    prehash::{Map, Set},
    Slot,
};

use massa_models::{BlockId, EndorsementId, SignedEndorsement, SignedHeader};
...
pub enum ProtocolPoolEvent {
    ...
    /// Get operations, for broadcasting to nodes.
...
pub enum ProtocolCommand {
    ...
    GetOperationsResults(Map<OperationId, Option<SignedOperation>>),
...
impl ProtocolCommandSender {
    ...
    pub async fn send_get_operations_results(
        ...
        let res = self
            .0
            .send(ProtocolCommand::GetOperationsResults(results))
            .await
            .map_err(|_| {
